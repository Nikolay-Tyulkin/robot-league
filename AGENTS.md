...
- Keep keyboard and touch input equivalent. Preserve increasing input sequences, single-use shoot/tap edges, pointer ownership, and clearing controls on cancellation, blur, pause, and navigation.
- Robot identities and names live in `ROBOT_KINDS`/`ROBOT_NAMES`. Preserve both online selections and per-player model instances, including mirror matches. Each online player changes only their own room selection; a changed pairing resets both ready flags, and choices lock after start. Solo can choose any robot or randomize among all three at each new match.
